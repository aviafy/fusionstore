...
orderSchema.methods.appendStatus = function appendStatus(code) {
  const targetIndex = ORDER_STATUSES.findIndex(status => status.code === code);
  if (targetIndex === -1) {
    return false;
  }

  const alreadyPresent = Array.isArray(this.statusHistory)
    ? this.statusHistory.some(status => status.code === code)
    : false;

  if (alreadyPresent) {
    this.statusIndex = Math.max(this.statusIndex || 0, targetIndex);
    return false;
  }

  const meta = ORDER_STATUSES[targetIndex];
  if (!meta) {
    return false;
  }
...
Order.STATUS_FLOW = ORDER_STATUSES;
