...
const seedDB = async ({ force = false, skipIfExists = true } = {}) => {
  ...
  products.push(...productSeeds);
  console.log(`Products data seeded successfully! Inserted ${productSeeds.length} records.`);

  return { seeded: true, skipped: false, inserted: productSeeds.length };
};

// Only runs when the "node productSeeds.js dev" command is executed manually
if (process.argv[2] == 'dev') {
  dotenv.config({ path: path.resolve(__dirname, '../.env') });
  seedDB({ force: true, skipIfExists: false }).then(() => {
    process.exit();
  });
}
